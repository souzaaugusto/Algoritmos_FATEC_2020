...
def squirrel_play(temp, is_summer):
  acrescento = 10 if is_summer else 0

  if temp >= 60 and temp <= 90 + acrescento:
    return True
  return False
...
def pego_correndo(speed, is_birthday):
  acrescento = 5 if is_birthday else 0

  if speed <= 60 + acrescento:
    return 0
  elif speed >= 60 + acrescento and speed <= 80 + acrescento:
    return 1
  else:
    return 2
