...
# near_ten(19) -> True
def near_ten(n):
  return (n % 10 >= 2 or n % 10 <= 8) and (n % 10 <= 2 or n % 10 >= 8) and (n % 10 > 2 or n % 10 < 8)
...
def double_char(s):
  elements_list = list(s)
  string = ''
  for car in elements_list:
    string += car * 2
  return string
...
def fila_tijolos(n_peq, n_gra, meta):
  if (n_peq >= meta % 7) and (n_peq + 7 * n_gra >= meta):
    return True
  else:
    return False
